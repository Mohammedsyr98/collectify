...
export function CustomerEditLoadingModal({
  onClose,
}: {
  onClose: () => void;
}) {
    ...
    >
      <section className="grid w-full max-w-[460px] gap-4 rounded-md border border-border bg-card p-5 shadow-[var(--shadow-md)]">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            aria-label={t('customers.edit.close')}
            className="inline-flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-[5px] border border-border bg-background text-muted-foreground transition duration-150 hover:bg-muted hover:text-foreground"
            onClick={onClose}
            type="button"
          >
            <X aria-hidden="true" size={16} strokeWidth={2.5} />
